...
def assign_boxes_to_levels_via_attn(
    attn_pool_features,
    pro_features,
):
    """
    ...
    Returns:
    """
    attn_pool_features = attn_pool_features.permute(1, 2, 0)
    pro_features = pro_features.permute(1, 0, 2)
    attn = torch.bmm(pro_features, attn_pool_features)
    attn = F.gumbel_softmax(attn, hard=True)
    index = nonzero_tuple(attn == 1)[-1]
    return index.to(torch.int64)
    # return max_level - min_level - index.to(torch.int64)

def assign_boxes_to_levels_via_attn_area(
    attn_pool_features,
    pro_features,
    box_lists: List[Boxes],
    min_level: int,
    ...
    # for the available feature maps
    level_assignments = torch.clamp(level_assignments, min=min_level, max=max_level)
    area_level = level_assignments.to(torch.int64) - min_level

    attn[torch.arange(attn.size(0)).unsqueeze(1), :, area_level.unsqueeze(1)] = \
        attn[torch.arange(attn.size(0)).unsqueeze(1), :, area_level.unsqueeze(1)] + torch.log2(box_sizes[:, None, None])
...
class AttnROIPooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], attn_pool_features, pro_features, box_lists: List[Boxes]):
        """
        Args:
        ...
        level_assignments = assign_boxes_to_levels_via_attn_area(
            attn_pool_features, pro_features, box_lists, self.min_level, self.max_level,
