...
class DeformROIPooler(nn.Module):
    """
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes], offset):
        """
        Args:
        ...
        if num_level_assignments == 1:
            return self.level_poolers[0](x[0], pooler_fmt_boxes, offset)
        ...
        for level, pooler in enumerate(self.level_poolers):
            ...
            if offset != None:
                offset_level = offset[inds]
            else:
                offset_level = None
            output[inds] = pooler(x[level], pooler_fmt_boxes_level, offset_level)
