...
class SparseRCNN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        proposal_boxes = proposal_boxes[None].repeat(images_whwh.size(0), 1, 1)

        # Prediction.
        outputs_class, outputs_coord = self.head(features, proposal_boxes, self.init_proposal_features.weight,
                                                 images_whwh, pos_imgs, self.pos_local)
        ...
        if self.training:
            ...
            box_pred = box_cxcywh_to_xyxy(box_pred) * images_whwh[None, :, :]
